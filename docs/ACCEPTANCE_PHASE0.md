...
| F) Webhook URL 確定 | LINE Console 設定確認 | URL が一致 | 一致 | LINE Console スクショ（2026-01-27） | PASS |
| G) 署名なし拒否 | `curl -i -X POST https://member-webhook-pvxgenwkba-ue.a.run.app/webhook/line -d '{}'` | 401 | 401 unauthorized | curl 出力 | PASS |
| H) Verify 成功 | LINE Console Verify + Cloud Logging | Verify成功 + acceptログ | Verify成功 + acceptログ確認 | LINE Console スクショ（2026-01-27）/ Cloud Logging | PASS |
