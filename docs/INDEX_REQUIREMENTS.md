...
- GitHub Actions から不足 index を apply するときは `.github/workflows/firestore-indexes-apply.yml` を `workflow_dispatch` で実行する。
- GitHub Actions apply の前提権限は deploy SA に `roles/datastore.indexAdmin` が付与されていること。
