...
- GitHub Actions から OIDC で不足indexを直接作成する場合は `Firestore Indexes Apply` workflow を `workflow_dispatch` で実行する。
- `Firestore Indexes Apply` は `docs/REPO_AUDIT_INPUTS/firestore_required_indexes.json` と実プロジェクトの差分を比較し、不足 composite index のみを idempotent に作成したあと `npm run firestore-indexes:check -- --project-id <PROJECT_ID>` で再検証する。
...
- Source:
  ...
  - `src/repos/firestore/emergencyRulesRepo.js`
...
- Query:
  ...
  - `where('enabled','==',...)` **or** `where('providerKey','==',...)` + `limit(...)`（rules）
- Note:
  - Emergency Layer は単一 where（status/regionKey/rules.enabled/rules.providerKey）のみを使用し、複合 where を導入しない。
