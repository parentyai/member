...
- admin operator affordances stay copy-only: `Copy next command`, `Copy remaining commands`, `Copy latest prompt path`, `Copy latest artifact path`, `Copy patch request path`, `Copy code apply task path`, `Copy code apply signoff path`, `Copy code apply record path`, `Copy code apply evidence path`, `Copy validation commands`, `Copy evidence prompt path`, `Copy evidence requirements`, `Copy expected outputs`, `Copy stop conditions`, `Copy operator instructions`, `Copy worktree path`, and `Copy candidate edit paths`
