...
- admin operator affordances stay copy-only: `Copy next command`, `Copy remaining commands`, `Copy latest prompt path`, `Copy latest artifact path`, `Copy validation commands`, `Copy evidence prompt path`, `Copy evidence requirements`, `Copy operator instructions`, `Copy worktree path`, and `Copy candidate edit paths`
