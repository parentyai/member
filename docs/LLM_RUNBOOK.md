...
- `PR-knowledge-hardening (r828)` 以降、medium/high-risk knowledge は `authorityTier` / `bindingLevel` / source refs / metadata completeness を source readiness 側で評価し、弱い根拠は `allow` にしない。
- `PR-knowledge-hardening (r828)` 以降、saved FAQ reuse の medium/high-risk 判定は linked source refs が弱い場合に `saved_faq_authority_below_threshold` / `saved_faq_freshness_below_threshold` / `saved_faq_source_snapshot_missing` を優先確認する。
- local 復旧確認コマンド:
