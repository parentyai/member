...
- admin quality patrol query: `success/completed`, `error/not_found`, `error/error`
  ...
  - nested `desktopPatrolSummary.promotionReview`: latest proposalId / review status / latest draft PR ref / latest review artifact kind / branch / worktree ref / patch draft ref / code edit task ref / code apply draft ref / code review packet ref / updatedAt
- nested `desktopPatrolSummary.promotionApproval`: approval stage / status / draft PR ref / worktree ref / `latestArtifactRef` / `latestPromptRef` / patch request ref / code apply task ref / code apply signoff ref / code apply record ref / `codeApplyEvidenceRef` / `codeApplyEvidencePromptRef` / validationCommandCount / candidateEditCount / operatorInstructionCount / evidenceRequirementCount / expectedOutputCount / stopConditionCount / nextCommand / remainingCommandCount / nextAction / updatedAt
  - UI actions: `Copy next command` / `Copy remaining commands` / `Copy latest prompt path` / `Copy latest artifact path` / `Copy patch request path` / `Copy code apply task path` / `Copy code apply signoff path` / `Copy code apply record path` / `Copy code apply evidence path` / `Copy validation commands` / `Copy evidence prompt path` / `Copy evidence requirements` / `Copy expected outputs` / `Copy stop conditions` / `Copy operator instructions` / `Copy worktree path` / `Copy candidate edit paths`
  - nested `desktopPatrolSummary.promotionBatch`: latest self-improvement batch run / completionStatus / queuedProposalCount / patchDraftReadyCount / blockedCaseIds / nextAction / updatedAt
  - audit `payloadSummary`: `desktopPatrolStatus`, `desktopPatrolStage`, `desktopPatrolQueueCount`, `desktopPatrolLatestRunId`, `desktopPatrolLastRunKind`, `desktopPatrolSendStatus`, `desktopPatrolPromotionProposalId`, `desktopPatrolPromotionKind`, `desktopPatrolPromotionStatus`, `desktopPatrolPromotionDraftPrRef`, `desktopPatrolPromotionUpdatedAt`, `desktopPatrolPromotionBatchRunId`, `desktopPatrolPromotionBatchCompletionStatus`, `desktopPatrolPromotionBatchQueuedProposalCount`, `desktopPatrolPromotionBatchPatchDraftReadyCount`, `desktopPatrolPromotionBatchBlockedCaseCount`, `desktopPatrolPromotionBatchNextAction`, `desktopPatrolPromotionBatchUpdatedAt`, `desktopPatrolPromotionReviewProposalId`, `desktopPatrolPromotionReviewStatus`, `desktopPatrolPromotionReviewKind`, `desktopPatrolPromotionReviewDraftPrRef`, `desktopPatrolPromotionReviewBranch`, `desktopPatrolPromotionReviewUpdatedAt`, `desktopPatrolPromotionApprovalStage`, `desktopPatrolPromotionApprovalStatus`, `desktopPatrolPromotionApprovalCommandCount`, `desktopPatrolPromotionApprovalEvidenceRequirementCount`, `desktopPatrolPromotionApprovalExpectedOutputCount`, `desktopPatrolPromotionApprovalStopConditionCount`, `desktopPatrolPromotionApprovalCandidateEditCount`, `desktopPatrolPromotionApprovalOperatorInstructionCount`, `desktopPatrolPromotionApprovalNextCommand`, `desktopPatrolPromotionApprovalNextAction`, `desktopPatrolPromotionApprovalRemainingCommandCount`, `desktopPatrolPromotionApprovalUpdatedAt`
- admin quality patrol approval plan: `success/completed`, `blocked/desktop_patrol_approval_unavailable`, `blocked/desktop_patrol_approval_latest_mismatch`, `blocked/desktop_patrol_approval_no_next_action`, `error/invalid_json`, `error/error`
- admin quality patrol approval execute: `success/completed`, `blocked/desktop_patrol_approval_plan_stale`, `blocked/desktop_patrol_approval_confirm_invalid`, `blocked/desktop_patrol_approval_local_only`, `error/planHash/confirmToken required`, `error/error`
  - route: `POST /api/admin/quality-patrol/desktop-approval/plan`
  - route: `POST /api/admin/quality-patrol/desktop-approval/execute`
  - execute は `managedFlowGuard + planHash + confirmToken` を必須とし、local 環境で approval artifact を 1 ステップだけ前進させる
