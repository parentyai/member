...
- nested `desktopPatrolSummary.promotionApproval`: approval stage / status / draft PR ref / worktree ref / `latestArtifactRef` / `latestPromptRef` / patch request ref / code apply task ref / code apply signoff ref / code apply record ref / validationCommandCount / candidateEditCount / operatorInstructionCount / nextCommand / remainingCommandCount / nextAction / updatedAt
  - UI actions: `Copy next command` / `Copy remaining commands` / `Copy latest prompt path` / `Copy latest artifact path` / `Copy validation commands` / `Copy operator instructions` / `Copy worktree path` / `Copy candidate edit paths`
