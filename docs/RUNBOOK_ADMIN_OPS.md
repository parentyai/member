...
- nested `desktopPatrolSummary.promotionApproval`: approval stage / status / draft PR ref / worktree ref / `latestArtifactRef` / `latestPromptRef` / patch request ref / code apply task ref / code apply signoff ref / code apply record ref / `codeApplyEvidenceRef` / `codeApplyEvidencePromptRef` / validationCommandCount / candidateEditCount / operatorInstructionCount / evidenceRequirementCount / expectedOutputCount / stopConditionCount / nextCommand / remainingCommandCount / nextAction / updatedAt
  - UI actions: `Copy next command` / `Copy remaining commands` / `Copy latest prompt path` / `Copy latest artifact path` / `Copy validation commands` / `Copy evidence prompt path` / `Copy evidence requirements` / `Copy operator instructions` / `Copy worktree path` / `Copy candidate edit paths`
