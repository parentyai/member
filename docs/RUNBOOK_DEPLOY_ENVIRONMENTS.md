...
## Firestore Index Apply Workflow
- 不足 Firestore composite index の remediation は `.github/workflows/firestore-indexes-apply.yml` を `workflow_dispatch` で使う。
- 前提:
  - `docs/REPO_AUDIT_INPUTS/firestore_required_indexes.json` が実運用SSOT
  - `vars.GCP_WIF_PROVIDER` と `vars.GCP_DEPLOY_SA` が設定済み
  - deploy SA に `roles/datastore.indexAdmin` が付与済み
- dry-run:
  - `gh workflow run firestore-indexes-apply.yml --ref main -f dry_run=true -f project_id=<PROJECT_ID>`
  - 期待: `npm run firestore-indexes:plan` だけ実行し、変更は行わない
- apply:
  - `gh workflow run firestore-indexes-apply.yml --ref main -f dry_run=false -f project_id=<PROJECT_ID>`
  - 期待: `npm run firestore-indexes:apply` の後に `npm run firestore-indexes:check` が PASS する
- 失敗時の切り分け:
  - `Auth (OIDC)` 失敗: WIF provider / SA impersonation 設定を確認
  - `Apply missing Firestore composite indexes` 失敗: deploy SA の `roles/datastore.indexAdmin` を確認
  - `Verify indexes aligned after apply` 失敗: Firestore 側の作成遅延か required 定義 mismatch を確認
