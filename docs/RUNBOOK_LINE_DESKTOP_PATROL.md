...
- operator surface can copy `nextCommand`, `remainingCommands`, `latestPromptRef.path`, `latestArtifactRef.path`, `validationCommands`, `codeApplyEvidencePromptRef.path`, `evidenceRequirements`, `expectedOutputs`, `stopConditions`, `operatorInstructions`, `worktreeRef.path`, and operator-visible `candidateEdits[].filePath` without adding new server-side write authority
