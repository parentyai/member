...
## Phase73x Add-only Data Contract（監査確定改善 A〜F）

### task_content_links
- collection: `task_content_links`
- document id: `ruleId`
- fields:
  - `ruleId`
  - `sourceTaskKey`
  ...
  - `createdAt`, `updatedAt`, `createdBy`, `updatedBy`
- write guard:
  - plan/set 二段階（planHash + confirmToken）
  - `ENABLE_TASK_CONTENT_LINK_MIGRATION_APPLY_V1=false` の場合 apply は 409 fail-close

### monitor/task-rules panel add-only controls
- migration controls:
  - `task-rules-task-content-link-plan`
  - `task-rules-task-content-link-apply`
  - `task-rules-task-content-link-manual-map-json`
- impact map controls:
  - `task-rules-link-impact-reload`
  - `task-rules-link-impact-limit`
  - `task-rules-link-impact-rows`

### Link impact API
- endpoint: `GET /api/admin/os/link-registry-impact`
- response summary:
  - `sharedIdCount`
  - `sharedWarnOrDisabledCount`
  - `referencedWarnOrDisabledCount`
- response items:
  - `id`, `state`, `warnOrDisabled`
  - `refs{task_contents,notifications,city_packs,vendor_facade}`
  - `domainCount`, `refCount`, `shared`

### Ops snapshot KPI add-only payload
- `taskUxAudit.richMenu.state`（`configured|unconfigured`）
- `taskUxAudit.readModelOverlap.overlapRatePct`
- `taskUxAudit.taskKeyLinkage.linkageRatePct`
- `taskUxAudit.linkRegistryImpact.sharedIdCount`
- `taskUxAudit.notification.notificationDeliveriesTotal`（SSOT）
- `taskUxAudit.continuation.continuationCompletionRatePct`
