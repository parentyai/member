...
- `Latest approval lane` は approval stage / status / draft PR ref / worktree ref / patch request ref / code apply task ref / code apply signoff ref / code apply record ref / validation command count / candidate edit count / operator instruction count / next command / remaining command count / next action / updatedAt を operator 向けに確認できる。
- `Latest approval lane` は `Copy next command` / `Copy remaining commands` / `Plan next step` / `Run next step` を表示できる。
