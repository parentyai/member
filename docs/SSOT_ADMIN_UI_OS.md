...
- `GET /api/admin/ops-feature-catalog-status`
  - 読み出し順は `ops_feature_status__catalog` を優先し、`listSnapshots({ snapshotType:'ops_feature_status' })` が失敗しても `catalog.rows` が存在する場合は `ok=true` を維持する。
  - `rowSource`（`catalog` / `docs`）と `warnings`（例: `ROW_DOCS_UNAVAILABLE`）を add-only で返し、部分読取を運用上識別可能にする。
- `POST /api/admin/ops-system-snapshot/rebuild`

### Retry Queue Recovery API（Phase73 add-only）
- `GET /api/phase73/retry-queue`
- `POST /api/phase73/retry-queue/plan`
- `POST /api/phase73/retry-queue/retry`
- `POST /api/phase73/retry-queue/give-up`
  - `queueId` / `planHash` / `confirmToken` を必須とし、`reason` は add-only 任意入力とする。
  - `send_retry_queue` は add-only で `status=GAVE_UP` と `giveUpReason` / `resolvedBy` / `resolvedAt` を保持する。
  - 即時停止は `ENABLE_RETRY_QUEUE_GIVEUP_V1=0`。
...
- `ENABLE_ADMIN_LEGACY_STATUS_V1`（既定: 1）
- `ENABLE_OPS_REALTIME_DASHBOARD_V1`（既定: 1）
- `ENABLE_OPS_SYSTEM_SNAPSHOT_V1`（既定: 1）
- `ENABLE_ADMIN_OPS_UI_V3`（既定: 1）
- `ENABLE_ADMIN_SYSTEM_CONSOLE_V1`（既定: 1）
- `ENABLE_ADMIN_COPY_V3`（既定: 1）
- `ENABLE_ADMIN_V3_CUTOVER`（既定: 1）
- `ENABLE_ADMIN_V3_KILL_SWITCH`（既定: 0）
- `quality patrol` の desktop summary は add-only で `Latest promotion` / `Latest review bundle` / `Latest approval lane` / `Latest promotion batch` を表示できる。
- `Latest approval lane` は approval stage / status / draft PR ref / worktree ref / latest artifact ref / latest prompt ref / patch request ref / code apply task ref / code apply signoff ref / code apply record ref / code apply evidence ref / code apply evidence prompt ref / validation command count / candidate edit count / operator instruction count / evidence requirement count / expected output count / stop condition count / next command / remaining command count / next action / updatedAt を operator 向けに確認できる。
- `Latest approval lane` は `Copy next command` / `Copy remaining commands` / `Copy latest prompt path` / `Copy latest artifact path` / `Copy patch request path` / `Copy code apply task path` / `Copy code apply signoff path` / `Copy code apply record path` / `Copy code apply evidence path` / `Copy validation commands` / `Copy evidence prompt path` / `Copy evidence requirements` / `Copy expected outputs` / `Copy stop conditions` / `Copy operator instructions` / `Copy worktree path` / `Copy candidate edit paths` / `Plan next step` / `Run next step` を表示できる。
- `Run next step` は `quality_patrol.desktop_approval.execute` として `managedFlowGuard + planHash + confirmToken` を必須にし、local-only で approval artifact を 1 ステップだけ前進させる。

## Admin UI v3 Shell Split（Phase879 add-only）
- `/admin/app` は route/pane 契約を維持したまま、visible shell を `Ops UI v3` と `System Console v1` に分ける。
- `Ops UI v3` は非エンジニア運用者向けの既定面とし、primary group を `Today / Messages / Members / Regional Ops / Recovery` に固定する。
- `System Console v1` は `settings / audit / diagnostics / maintenance / llm / developer-*` を集約する。
- 既存 `data-pane` / `route->pane` / compat redirect / role fallback / workbench boundary は変更しない。
- topbar は v3 では read-only shortcut を維持し、`search / scope / urgent alert` のみを表示する。
- `System / Evidence / Diagnostics / Trace / Policy / Raw` の語彙は ops first-view から外し、System Console へ退避する。
- rollback は `ENABLE_ADMIN_V3_KILL_SWITCH=1` で旧 shell へ即時復帰できることを契約とする。

## Ops First-View Noise Budget（Phase880 add-only）
- `Ops UI v3` の first fold では `primary CTA 1つ + secondary CTA 1つ` を上限とし、system/evidence/diagnostics の説明や raw block を出さない。
- `composer / monitor / read-model / city-pack / emergency-layer` では `data-v3-ops-hidden="true"` を用いて system-console 向け要素を退避し、`data-v3-advanced-filter="true"` で advanced filter を初期面から外す。
- decision card の文言は pane ごとに `動詞 + 結果` に言い換え、ops shell では不要な tertiary action を非表示にできることを契約とする。
- `System Console v1` へ移した要素は route/pane id を変えず、deep link と role fallback を維持したまま表示文脈だけを分離する。

## Home / Alerts Task-First Surface（Phase881 add-only）
- `home` は `最初にやること / 次に確認すること / 判断材料` の順で表示し、KPI の詳細は `home-kpi-details` の disclosure 配下へ退避する。
- `home` の hero action は `要対応を確認する` を primary、`配信結果を見る` を secondary とし、`composer / audit / system` への direct CTA は first-view に置かない。
- `alerts` は `decision-card + priority-summary + table` の3層構造にし、hero から最初の案件へ遷移できることを契約とする。
- `home-kpi-details` は `data-admin-fold-allowed="true"` により、global no-collapse 契約を壊さず局所的に折りたたみ可能とする。
- `home / alerts` の説明文は `何をするか / いつ使うか / 触るとどうなるか` を満たす task-first copy に固定する。

## Members / Regional Ops Task-First Surface（Phase882 add-only）
- `read-model / city-pack / emergency-layer` は decision-card の直下に task-summary panel を置き、`表示件数 / 注意件数 / 最初にやること / 次に確認すること` を first-view で示す。
- `read-model` は `この画面でやること` の補助説明を維持しつつ、会員一覧へ入る前に `確認が必要な会員 / 有料会員 / 次の操作` を判断できる構造にする。
- `city-pack` は `要確認 / 停止中 / 表示中` を先に示し、primary action は detail rail を開くのではなく `要確認の候補一覧` へ入る導線に固定する。
- `emergency-layer` は `Emergency Inbox` を主面とし、provider / rules / evidence / trace は `data-v3-ops-hidden="true"` で ops first-view から退避する。
- `emergency` の bulletin table は ops shell では evidence / trace 列と evidence action を表示せず、判断待ち件数と status filter を先に扱う。
- この phase でも `/admin/app?pane=*`, `data-pane`, workbench zone, deep link, role fallback は変更しない。

## Composer / Monitor / Errors Task-First Workbench（Phase883 add-only）
- `composer / monitor / errors` も decision-card の直下に task-summary panel を置き、`今の状態 / 最初にやること / 次に確認すること` を first-view で固定する。
- `composer` は ops shell で `scenario-step overview / trigger-order note / state-machine / evidence meta` を `data-v3-ops-hidden="true"` へ寄せ、主面では `本文を整える -> 次の操作へ進む` の順だけを見せる。
- `monitor` は ops shell で `snapshot / fallback / readLimit / trace` などの診断条件を first-view から外し、`一覧条件を絞る -> 異常のある配信を見る` を優先する。
- `errors` は tertiary CTA を first-view から外し、`危険リンク / 再送待ち / 次の対応先` の3点だけで判断できる構造にする。
- `errors-summary-details` と trace 入力は ops shell では hidden のまま残し、route・id・evidence path は維持したまま表示文脈だけを分離する。
- この phase でも `/admin/app?pane=*`, `data-pane`, workbench zone, deep link, role fallback は変更しない。

## Operator Walkthrough Polish（Phase888 add-only）
- この phase は `home -> alerts`, `alerts -> composer -> monitor`, `home/alerts -> errors`, `home/alerts -> read-model`, `home/alerts -> city-pack / emergency-layer` の 5 本を canonical walkthrough として固定する。
- 各 ops pane の first-view contract は `主目的を1行で読める / primary CTA 1つ / secondary CTA 1つまで / open details 0 / system jargon 0` とする。
- `composer / monitor / errors / emergency-layer` の dense details は ops shell では `data-workbench-collapsible="true"` かつ default closed とし、明示操作時だけ開く。
- `read-model` は ops shell で operator 向け列セットだけを first-view へ出し、technical columns は route/id を残したまま詳細側へ退避する。
- `System Console` 配下の pane は `Overview / Warnings / Actions / Details / Raw` の順で section を並べ、operator 面から raw へ直接飛ばない。
- topbar v3 の第一列は `search / scope / urgent` を優先し、role switch は secondary row に退避する。

## Operator Minimal UI Reset（Phase890 add-only）
- operator primary nav は `home / alerts / read-model / city-pack / llm` の 5 本に固定し、visible label は `ダッシュボード / 通知配信状況 / 会員情報 / City Pack・緊急レイヤー / FAQ` とする。
- `/admin/* -> /admin/app?pane=*`, `data-pane`, role fallback, workbench boundary, 既存 v3 flags は変更しない。
- `home` は利用状況ダッシュボードとして扱い、first-view KPI は `登録者数 / 有料ユーザー数 / 反応率 / エンゲージメント / FAQ稼働状況` の 5 つに固定する。
- `alerts` の row click は deterministic に `composer / monitor / read-model / city-pack(guide) / city-pack(emergency) / errors / maintenance` へ遷移し、operator はここから着手先を決める。
- `city-pack` は operator shell で `guide | emergency` mode を持ち、`emergency-layer` は deep-link 互換だけ残して primary nav から外す。
- `llm` は operator shell では FAQ surface として扱い、AI/LLM 用語は first-view から排除する。
- operator terminal action は `window.confirm` を使わず、`現在登録されている内容 / 実行後の状態 / 変更される項目 / 影響件数 / 完了後に移動する画面 / 取り消し可否` を持つ preview modal を経由する。
- delete 系で backend が retire/非表示を使う場合、operator copy は `削除する（履歴を残す）` に固定する。
- `System Console` は `Overview / Warnings / Actions / Details / Raw` 順の section order を維持し、operator から raw に直接飛ばさない。

## Operator Hard Reset Acceptance（Phase893 add-only）
- `CITY_PACK_OPERATOR_MODE` は state 初期化より前に定義し、`/admin/app?pane=*` と legacy redirect で `ReferenceError` を起こさない。
- topbar の profile menu は operator shell の初期表示で自動展開しない。`expandPaneDetails` / `expandAllDetails` / `enforceNoCollapseUi` は `data-topbar-menu` を auto-open 対象から除外する。
- `read-model` の operator detail rail は `会員ID / 料金プラン / 課金状態 / 契約期限 / 最終利用 / 次の確認 / 未完了タスク / 期限超過 / 次タスク期限` を first-view に残し、Stripe / usage / blocked history / Journey secondary details は ops shell で隠す。
- `read-model` の operator first-view table は `確認する会員 / 料金プラン / 契約状態 / 直近反応 / 問い合わせ回数 / 次の確認` の 6 列固定とし、sort button は static label 化して visible button count を増やさない。
- `read-model` は operator shell で初期表示時に会員を自動選択しない。detail rail は会員選択後にだけ開く。
- `FAQ` の operator first-view は `新しい質問を登録する` を primary とし、`公開を停止する / 削除する（履歴を残す）` は既存FAQを選んだときだけ表示する。
- `FAQ` は first-view で destructive action を初期HTMLから hidden にし、preview 前に誤って押せない状態で始める。
- Playwright acceptance gate の最小条件は `pageErrors=0`, `menuOpen=0`, `activePane 正常`, `operator first-view banned term=0`, `FAQ / City Pack preview modal に影響件数・影響対象・終点・取り消し可否が出る` とする。
- operator first-view の chrome は `検索 / スコープ / 要対応` を最上段に固定し、見出しラベルは可能な限り削って作業語を優先する。

### Legacy Routes Surface（P0 add-only）
- `ENABLE_ADMIN_LEGACY_STATUS_V1=1` のとき、`LEGACY導線` パネルを `admin/developer` role へ表示する。
- パネルは read-only（一覧 + 注意書き）とし、legacy route 実行操作は追加しない。
- API契約: `GET /api/admin/legacy-status` は `path/successor/caution/ssotRef` を返す。
...
- `window.ENABLE_ADMIN_LOCAL_PREFLIGHT_BLOCKING_V1` を boot script へ注入する（既定: `false`）。
...
- local strict SA policy: `ENABLE_ADMIN_LOCAL_PREFLIGHT_STRICT_SA_V1`（既定: local=OFF / non-local=OFF）
...
- local SA key auto fallback: `ENABLE_ADMIN_LOCAL_PREFLIGHT_AUTO_SA_V1`（既定: local=ON / non-local=OFF）
  - `GOOGLE_APPLICATION_CREDENTIALS` 未設定時、`ADMIN_LOCAL_PREFLIGHT_SA_KEY_PATH`（任意）→ `$HOME/.secrets/member-dev-sa.json` の順で読み取り可能な鍵を探索する
  - 有効な鍵が見つかった場合は `GOOGLE_APPLICATION_CREDENTIALS` へ自動適用し、同一プロセス内の preflight/probe 判定で利用する
  - 自動適用を停止したい場合は `ENABLE_ADMIN_LOCAL_PREFLIGHT_AUTO_SA_V1=0` を設定する
...
- 既定は non-blocking とし、preflight未復旧でも初期Firestore依存ロードは継続する（バナー警告のみ）。
- `ENABLE_ADMIN_LOCAL_PREFLIGHT_BLOCKING_V1=1` 時のみ `degraded` モードで初期Firestore依存ロードを抑止し、Dashboard KPIを `BLOCKED` 表示に統一する。
- preflight復旧時は初期ロードを自動再開する。

### SA鍵発行不可ポリシー時の運用プロファイル（add-only）
- 対象: 組織ポリシーでローカルSA鍵を新規発行できない環境。
- `ENABLE_ADMIN_LOCAL_PREFLIGHT_STRICT_SA_V1` は local 既定 `OFF` とし、ADC経路で preflight probe を継続する。
- strict SA を使う必要がある場合のみ `ENABLE_ADMIN_LOCAL_PREFLIGHT_STRICT_SA_V1=1` を明示設定して有効化する。
- `SA_KEY_REQUIRED` の影響説明は「初期ロード停止」ではなく「Firestore依存APIの認証失敗により NOT AVAILABLE が増える可能性」に統一する。

### Dashboard KPI 読み取り契約（add-only）
- `/admin/app` の Dashboard KPI 取得は `fallbackMode=allow&fallbackOnEmpty=true` を既定とする。
- Dashboard の `再読み込み` 操作は `snapshotRefresh=true` を付与して再計算を優先し、画面キャッシュ（`dashboardCacheByMonths`）を破棄して最新値を取得する。
- 旧互換の `fallbackMode=block` は URL クエリ `dashboardFallbackMode=block` のオプトインでのみ使用する。
- 目的: データ疎な期間でも 12カードを `NOT AVAILABLE` 全面表示へ倒さず、計算可能な範囲を継続表示する。
...
## LLM Quality Loop v2 Board（add-only）
- `pane-llm` の既存 DOM id / route id / one-click trace flow を変更しない。
- `LLM Quality Loop v2` は既存 quality board の下に read-only で追加する。
- 少なくとも以下を表示する:
  - integration KPI
  - critical slice board
...
- Safety:
  - `set/template-set/apply` は `planHash + confirmToken` 必須
  - write API は managed flow action（`task_rules.set` / `task_rules.template_set` / `task_rules.apply`）で保護
