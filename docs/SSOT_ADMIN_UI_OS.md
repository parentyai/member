...
- `quality patrol` の desktop summary は add-only で `Latest promotion` / `Latest review bundle` / `Latest approval lane` / `Latest promotion batch` を read-only 表示できる。
...
- `Latest approval lane` は copy-only command actions として `Copy next command` / `Copy remaining commands` を表示できるが、server-side mutation は追加しない。
