...
- `Latest approval lane` は `Copy next command` / `Copy remaining commands` / `Copy latest prompt path` / `Copy latest artifact path` / `Copy validation commands` / `Copy operator instructions` / `Plan next step` / `Run next step` を表示できる。
