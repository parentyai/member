...
- `Latest approval lane` は `Copy next command` / `Copy remaining commands` / `Copy latest prompt path` / `Copy latest artifact path` / `Copy validation commands` / `Copy evidence prompt path` / `Copy evidence requirements` / `Copy operator instructions` / `Copy worktree path` / `Copy candidate edit paths` / `Plan next step` / `Run next step` を表示できる。
