...
| conversation_review_snapshots | event | 180d | CONDITIONAL | YES |
| journey_templates | config | INDEFINITE | NO | NO |
...
| quality_issue_registry | config | INDEFINITE | NO | NO |
| quality_improvement_backlog | config | INDEFINITE | NO | NO |

Note: Quality Patrol review units are derived read-only outputs in PR-2. They are not persisted and therefore inherit retention from `conversation_review_snapshots`, `llm_action_logs`, `faq_answer_logs`, and trace bundle reads.
| source_audit_runs | transient | 30d | CONDITIONAL | YES |
| system_flags | config | INDEFINITE | NO | NO |
| task_events | evidence | 365d | NO | NO |
| step_rules | config | INDEFINITE | NO | NO |
| tasks | config | INDEFINITE | NO | NO |
| step_rule_change_logs | evidence | 365d | NO | NO |
