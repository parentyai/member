...
function isLowFrictionDirectPrompt(messageText) {
  ...
  return /((最初に確認すること|最初の確認事項|最初の確認先).*((2つ|二つ|2点|二点)|(1つ|一つ)))|((その2点|その二点).*(先に確認する方).*((1つ|一つ)だけ))|((公式確認が必要になる点).*((1つ|一つ)だけ))|((必要書類).*((2つ|二つ|2点|二点)だけ))|((予約が必要かどうか).*(どこを見れば).*(1文|一文))|((今日やること).*((1個|一個|1つ|一つ)だけ))|((最後に).*(2行|二行)でまとめて)/i.test(text);
}
