...
function resolveAdminLocalPreflightBlockingFlag() {
  return resolveBooleanEnvFlag('ENABLE_ADMIN_LOCAL_PREFLIGHT_BLOCKING_V1', false);
}
...
function resolveAdminHomeCleanSurfaceFlag() {
  return resolveBooleanEnvFlag('ENABLE_ADMIN_HOME_CLEAN_SURFACE_V1', true);
}
...
function resolveComposerAbOptionFlag() {
...
function resolveAdminBuildMeta() {
  ...
  return {
    ...
    composerAbOptionEnabled: resolveComposerAbOptionFlag()
  };
}
...
function buildAdminAppBootScript() {
  ...
  const localPreflightBlockingEnabled = resolveAdminLocalPreflightBlockingFlag();
...
<html lang="en">
  ...
  if (req.method === 'POST' && pathname === '/internal/jobs/llm-action-reward-finalize') {
    let bytes = 0;
    const chunks = [];
    let tooLarge = false;
    const collectBody = () => new Promise((resolve) => {
    ...
    (async () => {
      const { handleLlmActionRewardFinalizeJob } = require('./routes/internal/llmActionRewardFinalizeJob');
      const body = await collectBody();
      await handleLlmActionRewardFinalizeJob(req, res, body);
    })().catch(() => {
      res.writeHead(500, { 'content-type': 'application/json; charset=utf-8' });
      res.end(JSON.stringify({ ok: false, error: 'error' }));
    });
  ...
  if (req.method === 'POST' && pathname === '/internal/jobs/canonical-core-outbox-sync') {
    let bytes = 0;
    const chunks = [];
    let tooLarge = false;
    const collectBody = () => new Promise((resolve) => {
    ...
    (async () => {
      const { handleCanonicalCoreOutboxSyncJob } = require('./routes/internal/canonicalCoreOutboxSyncJob');
      const body = await collectBody();
      await handleCanonicalCoreOutboxSyncJob(req, res, body);
    })().catch(() => {
      res.writeHead(500, { 'content-type': 'application/json; charset=utf-8' });
      res.end(JSON.stringify({ ok: false, error: 'error' }));
    });
