...
function normalizeDailyKpi(dateKey, data) {
  ...
  return {
    ...
    detailOpenToContinueRate: normalizeRate(payload.detailOpenToContinueRate),
    detailOpenToCompleteRate: normalizeRate(payload.detailOpenToCompleteRate),
    detailContinueToCompleteRate: normalizeRate(payload.detailContinueToCompleteRate),
    deliveryToDetailToDoneRate: normalizeRate(payload.deliveryToDetailToDoneRate),
    ...
    detailOpenCount: Math.max(0, Math.floor(normalizeNumber(payload.detailOpenCount, 0))),
    detailSectionOpenCount: Math.max(0, Math.floor(normalizeNumber(payload.detailSectionOpenCount, 0))),
    detailContinueCount: Math.max(0, Math.floor(normalizeNumber(payload.detailContinueCount, 0))),
    detailCompleteCount: Math.max(0, Math.floor(normalizeNumber(payload.detailCompleteCount, 0))),
    detailOpenedAttributedCount: Math.max(0, Math.floor(normalizeNumber(payload.detailOpenedAttributedCount, 0))),
    detailCompletedAttributedCount: Math.max(0, Math.floor(normalizeNumber(payload.detailCompletedAttributedCount, 0))),
