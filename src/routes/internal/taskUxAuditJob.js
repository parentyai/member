...
async function handleTaskUxAuditJob(req, res, bodyText) {
  if (req.method !== 'POST') {
  ...
  if (!isTaskUxAuditKpiEnabled()) {
    writeJson(res, 409, { ok: false, error: 'task_ux_audit_kpi_disabled' });
    return;
  }
