...
test('getChecklistForUser: returns checklists for scenario/step', async () => {
  ...
  assert.strictEqual(result.step, '3mo');
  assert.strictEqual(result.checklists.length, 1);
});

test('getChecklistForUser: missing step returns empty', async () => {
  await usersRepo.createUser('U1', { scenario: 'A' });
  const result = await getChecklistForUser({ lineUserId: 'U1' });
  assert.strictEqual(result.checklists.length, 0);
});
