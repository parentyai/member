...
test('phase228: /api/phaseLLM2/ops-explain and /api/phaseLLM3/ops-next-actions require admin token (fail-closed)', async (t) => {
  ...
  const explainNoToken = await request({ port, method: 'GET', path: '/api/phaseLLM2/ops-explain' });
  assert.strictEqual(explainNoToken.status, 401);

  const nextNoToken = await request({ port, method: 'GET', path: '/api/phaseLLM3/ops-next-actions' });
  assert.strictEqual(nextNoToken.status, 401);

  const headers = { 'x-admin-token': 'test_admin_token' };

  const explainAuthed = await request({ port, method: 'GET', path: '/api/phaseLLM2/ops-explain', headers });
  ...
  const nextAuthed = await request({ port, method: 'GET', path: '/api/phaseLLM3/ops-next-actions', headers });
