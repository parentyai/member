...
test('phase241: all ui.* keys used in /admin/app are present in dictionary', () => {
  ...
  const htmlPlaceholderKeys = extractKeysByRegex(appHtml, /data-dict-placeholder="([^"]+)"/g);
  const jsKeys = extractKeysByRegex(appJs, /t\('([^']+)'/g);

  const usedUiKeys = new Set();
  [htmlDictKeys, htmlTipKeys, htmlPlaceholderKeys, jsKeys].forEach((set) => {
    set.forEach((key) => {
