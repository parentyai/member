...
test('phase271: city-pack-bulletins routes require admin token', async (t) => {
  ...
  await cityPackBulletinsRepo.createBulletin({
    ...
    notificationId: 'n_phase271',
    summary: 'Phase271 bulletin',
    traceId: 'trace_phase271'
  });

  const noToken = await request({
    port,
    method: 'GET',
    path: '/api/admin/city-pack-bulletins'
  });
  ...
  const withToken = await request({
    port,
    method: 'GET',
    path: '/api/admin/city-pack-bulletins?limit=5',
    headers: {
      'x-admin-token': 'phase271_admin_token',
