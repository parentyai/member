...
test('phase308: top3 analytics hotspots use explicit bounded limits', () => {
  ...
  assert.ok(dashboard.includes('analyticsReadRepo.listAllEvents({ limit: scanLimit })'));
  assert.ok(dashboard.includes('analyticsReadRepo.listAllNotificationDeliveries({ limit: scanLimit })'));
  ...
  assert.ok(userSummary.includes('listAllUsers({ limit: analyticsLimit })'));
