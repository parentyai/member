...
test('phase311: /api/admin/repo-map remains admin-guarded and returns layers', async (t) => {
  ...
  process.env.ADMIN_OS_TOKEN = 'phase311_admin_token';
  ...
  const ok = await request({
    ...
    path: '/api/admin/repo-map',
    headers: {
      'x-admin-token': 'phase311_admin_token',
      'x-actor': 'phase311_test',
      'x-trace-id': 'trace_phase311_repo_map'
    }
  });
  assert.strictEqual(ok.status, 200, ok.body);
  const payload = JSON.parse(ok.body);
  assert.strictEqual(payload.ok, true);
  assert.ok(payload.repoMap && payload.repoMap.meta);
  assert.ok(payload.layers && payload.layers.operational && payload.layers.developer && payload.layers.communication);
});
