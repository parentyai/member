...
test('phase674: page header keeps primary CTA mapping for home workbench entry', () => {
  const js = fs.readFileSync('apps/admin/assets/admin_app.js', 'utf8');
  const css = fs.readFileSync('apps/admin/assets/admin.css', 'utf8');

  assert.ok(js.includes('const PAGE_HEADER_ACTION_MAP = Object.freeze('));
  assert.ok(js.includes("labelKey: 'ui.label.decision.action.createEdit'"));
  assert.ok(js.includes("paneTarget: 'composer'"));
