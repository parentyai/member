...
const SECRET = 'phase725_line_secret';
...
function loadWebhookWithStubs() {
  ...
  setOverride('../../src/repos/firestore/systemFlagsRepo', {
    ...
    getLlmBanditEnabled: async () => true
  });
    ...
    composeConciergeReply: async () => ({
      ...
      auditMeta: {
        ...
        counterfactualTopArms: [
          { rank: 1, armId: 'Checklist|cta=1', styleId: 'Checklist', ctaCount: 1, score: 0.77 },
          { rank: 2, armId: 'Coach|cta=1', styleId: 'Coach', ctaCount: 1, score: 0.71 }
        ]
      }
    })
  });
...
function createWebhookBody(text) {
  return JSON.stringify({
    events: [
      {
        type: 'message',
        replyToken: 'rt_phase725',
        source: { userId: 'U_PHASE725' },
...
test('phase725: webhook and llm_action_logs propagate contextual and counterfactual metadata', async (t) => {
  ...
  assert.ok(gateAudit);
  assert.equal(gateAudit.payloadSummary.counterfactualSelectedRank, 1);
  assert.ok(Array.isArray(gateAudit.payloadSummary.counterfactualTopArms));
  assert.equal(gateAudit.payloadSummary.counterfactualTopArms.length, 2);
  assert.ok(gateAudit.payloadSummary.contextualFeatures);

  assert.equal(loaded.actionLogs.length, 1);
  assert.equal(loaded.actionLogs[0].requestId, 'phase725_req_1');
  assert.equal(loaded.actionLogs[0].counterfactualSelectedArmId, 'Checklist|cta=1');
  assert.equal(loaded.actionLogs[0].counterfactualSelectedRank, 1);
  assert.ok(Array.isArray(loaded.actionLogs[0].counterfactualTopArms));
  assert.ok(loaded.actionLogs[0].contextualFeatures);
});
