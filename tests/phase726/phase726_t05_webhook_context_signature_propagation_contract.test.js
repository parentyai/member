...
const SECRET = 'phase726_line_secret';
...
function loadWebhookWithStubs() {
  ...
  setOverride('../../src/usecases/users/ensureUser', { ensureUserFromWebhook: async () => ({ ok: true }) });
  setOverride('../../src/usecases/notifications/sendWelcomeMessage', { sendWelcomeMessage: async () => ({ skipped: true }) });
  setOverride('../../src/usecases/line/logLineWebhookEvents', { logLineWebhookEventsBestEffort: async () => ({ ok: true }) });
  setOverride('../../src/usecases/users/declareRedacMembershipIdFromLine', { declareRedacMembershipIdFromLine: async () => ({ status: 'none' }) });
  setOverride('../../src/usecases/users/getRedacMembershipStatusForLine', { getRedacMembershipStatusForLine: async () => ({ status: 'NOT_DECLARED' }) });
  setOverride('../../src/infra/lineClient', { replyMessage: async () => ({ status: 200 }) });
  setOverride('../../src/usecases/cityPack/declareCityRegionFromLine', { declareCityRegionFromLine: async () => ({ status: 'none' }) });
  setOverride('../../src/usecases/cityPack/declareCityPackFeedbackFromLine', { declareCityPackFeedbackFromLine: async () => ({ status: 'none' }) });
  setOverride('../../src/usecases/llm/recordUserLlmConsent', { recordUserLlmConsent: async () => ({ ok: true }) });
  setOverride('../../src/repos/firestore/eventsRepo', { createEvent: async () => ({ ok: true }) });
    ...
    composeConciergeReply: async () => ({
      ...
      auditMeta: {
        ...
        chosenAction: {
          ...
          questionFlag: false,
          selectionSource: 'bandit_contextual_exploit',
          score: 0.77,
          scoreBreakdown: { base: 0.5 }
        },
        contextVersion: 'concierge_ctx_v1',
        featureHash: 'feature_726',
        contextSignature: 'ctxsig_v1_xxx',
        contextualBanditEnabled: true,
        postRenderLint: { findings: [], modified: false },
        contextualFeatures: { featureVersion: 'bandit_ctx_v1', journeyPhase: 'pre' },
        counterfactualSelectedArmId: 'Checklist|cta=1',
        counterfactualSelectedRank: 1,
        counterfactualTopArms: [{ rank: 1, armId: 'Checklist|cta=1', styleId: 'Checklist', ctaCount: 1, score: 0.77 }]
      }
    })
  });
  setOverride('../../src/repos/firestore/faqArticlesRepo', { getArticle: async () => null });
  setOverride('../../src/repos/firestore/linkRegistryRepo', { getLink: async () => null });
  setOverride('../../src/repos/firestore/sourceRefsRepo', { getSourceRef: async () => null });
  setOverride('../../src/repos/firestore/cityPacksRepo', { getCityPack: async () => null });
...
function createWebhookBody(text) {
  return JSON.stringify({
    events: [
      {
        type: 'message',
        replyToken: 'rt_phase726',
        source: { userId: 'U_PHASE726' },
...
test('phase726: webhook audit and llm_action_logs include context signature fields', async (t) => {
  ...
  assert.ok(gateAudit);
  assert.equal(gateAudit.payloadSummary.contextSignature, 'ctxsig_v1_xxx');
  assert.equal(gateAudit.payloadSummary.contextualBanditEnabled, true);

  assert.equal(loaded.actionLogs.length, 1);
  assert.equal(loaded.actionLogs[0].contextSignature, 'ctxsig_v1_xxx');
  assert.equal(loaded.actionLogs[0].contextualBanditEnabled, true);
});
