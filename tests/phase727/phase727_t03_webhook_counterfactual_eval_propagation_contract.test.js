...
const SECRET = 'phase727_line_secret';
  ...
  setOverride('../../src/usecases/assistant/concierge/composeConciergeReply', {
    composeConciergeReply: async () => ({
      ...
      auditMeta: {
        ...
        featureHash: 'feature_727',
        contextSignature: 'ctxsig_v1_727',
        ...
        counterfactualTopArms: [{ rank: 1, armId: 'Coach|cta=1', styleId: 'Coach', ctaCount: 1, score: 0.89 }],
        counterfactualEval: {
          version: 'v1',
          eligible: true,
          selectedArmId: 'Checklist|cta=1',
          selectedRank: 2,
          bestArmId: 'Coach|cta=1',
          bestScore: 0.89,
          selectedScore: 0.77,
          scoreGap: 0.12,
          minGap: 0.12,
...
function createWebhookBody(text) {
  return JSON.stringify({
    events: [
      {
        type: 'message',
        replyToken: 'rt_phase727',
        source: { userId: 'U_PHASE727' },
...
test('phase727: webhook audit and llm_action_logs include counterfactualEval fields', async (t) => {
  ...
  assert.ok(gateAudit);
  assert.ok(gateAudit.payloadSummary.counterfactualEval);
  assert.equal(gateAudit.payloadSummary.counterfactualEval.opportunityDetected, true);

  assert.equal(loaded.actionLogs.length, 1);
  assert.ok(loaded.actionLogs[0].counterfactualEval);
  assert.equal(loaded.actionLogs[0].counterfactualEval.bestArmId, 'Coach|cta=1');
});
