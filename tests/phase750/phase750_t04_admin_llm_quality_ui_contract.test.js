...
test('phase750: admin app renders quality framework dashboard from usage summary', () => {
  ...
  assert.match(appJs, /function renderLlmRawSourceReview\(/);
  assert.match(appJs, /function renderLlmProcedureKnowledgeReview\(/);
  ...
  assert.match(appJs, /renderLlmRawSourceReview\(data && data\.summary \? data\.summary : null\)/);
  assert.match(appJs, /renderLlmProcedureKnowledgeReview\(data && data\.summary \? data\.summary : null\)/);
  assert.match(appJs, /llm-raw-source-review/);
  assert.match(appJs, /llm-procedure-knowledge-review/);
