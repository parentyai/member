...
test('phase769: strict release policy fails when runtime conversation signals are missing', () => {
  ...
  assert.equal(payload.runtimeSignalCoverage.requiredKeys.includes('internalLabelLeakRate'), true);
  assert.equal(payload.runtimeSignalCoverage.requiredKeys.includes('citySpecificityResolvedRate'), true);
