...
test('phase855: metrics artifact surfaces decay-aware ops gate without changing transcript coverage contract', async () => {
  const outputPath = tempJsonPath('metrics_decay_ops_gate');
    ...
    buildConversationReviewUnitsFromSources: async (params) => {
      if (params && params.fromAt === '2026-03-15T12:51:00.000Z' && params.toAt === '2026-03-15T12:55:00.000Z') {
        return {
          ...
          llmActionLogs: fullReviewUnits.slice(1).map((unit) => ({
            createdAt: unit.sourceWindow.toAt,
            transcriptSnapshotOutcome: 'written'
          })),
          transcriptCoverage: {
            observedCount: 5,
      ...
      return {
        ...
        llmActionLogs: fullReviewUnits.map((unit) => ({
          createdAt: unit.sourceWindow.toAt,
          transcriptSnapshotOutcome: 'written'
        })),
        transcriptCoverage: {
          ...
          snapshotInputDiagnostics: {
            ...
            masking_removed_text: 0,
            region_prompt_fallback: 0,
            assistantReplyPresent: { trueCount: 49, falseCount: 11 },
            assistantReplyLength: { observedCount: 60, min: 0, max: 90, avg: 43.5 },
            sanitizedReplyLength: { observedCount: 60, min: 0, max: 90, avg: 43.5 },
            snapshotBuildAttempted: { trueCount: 60, falseCount: 0 },
            snapshotBuildSkippedReason: {
              feature_flag_off: 0,
              line_user_key_missing: 0,
              assistant_reply_missing: 11,
        ...
        sourceCollections: ['llm_action_logs'],
        counts: { snapshots: 6, llmActionLogs: 65, faqAnswerLogs: 10, traceBundles: 6 },
  ...
  assert.ok(artifact.decayAwareOpsGate);
  assert.equal(artifact.decayAwareOpsGate.decision, 'NO_GO');
  assert.equal(artifact.decayAwareOpsGate.decisionReasonCode, 'historical_backlog_dominant');
  assert.equal(artifact.decayAwareOpsGate.operatorAction, 'separate_historical_backlog_from_current_runtime');
  assert.equal(artifact.decayAwareOpsGate.prDEligible, false);
  assert.equal(artifact.decayAwareOpsGate.prDReasonCode, 'historical_backlog_present');
