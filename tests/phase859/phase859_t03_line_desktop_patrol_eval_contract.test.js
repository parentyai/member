...
test('phase859: desktop patrol eval CLI converts dry-run traces into read-only quality patrol artifacts', () => {
  ...
  assert.match(result.outputPath, /artifacts\/line_desktop_patrol\/evals\/.*\/desktop_patrol_eval\.json$/);
  assert.ok(fs.existsSync(result.outputPath));
  assert.ok(fs.existsSync(planningOutput));

  const mainArtifact = JSON.parse(fs.readFileSync(result.outputPath, 'utf8'));
