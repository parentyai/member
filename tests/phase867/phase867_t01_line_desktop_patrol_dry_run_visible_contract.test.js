...
test('phase867: dry-run harness records visible_after when visible message read succeeds', (t) => {
  const tempRoot = makeTempRoot('phase867-dry-run-visible-');
...
class FakeAdapter:
    ...
    def execute_read_visible_messages(self, output_path, target_process_name="LINE", max_items=20, timeout_seconds=2.0):
        ...
        payload = {
            ...
            "truncated": False,
            "items": [
                {"index": 1, "role": "unknown", "text": "前回の案内を確認しました"},
                {"index": 2, "role": "unknown", "text": "次の手順を教えてください"},
            ],
        }
        output_path.write_text(json.dumps(payload), encoding="utf-8")
        return {
...
`;
  ...
  assert.deepEqual(trace.visible_after, [
    { role: 'unknown', text: '前回の案内を確認しました' },
    { role: 'unknown', text: '次の手順を教えてください' }
  ]);
  assert.equal(trace.observation_artifacts.read_visible_messages.result.status, 'ok');
  assert.ok(typeof trace.observation_artifacts.read_visible_messages.output_path === 'string' && trace.observation_artifacts.read_visible_messages.output_path.endsWith('/after.visible.json'));
  assert.ok(fs.existsSync(trace.observation_artifacts.read_visible_messages.output_path));
});
