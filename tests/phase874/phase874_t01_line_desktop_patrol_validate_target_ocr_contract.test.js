...
test('phase874: execute_validate_target tolerates OCR dakuten noise in the LINE header title', () => {
  const code = `
import json
from member_line_patrol.macos_adapter import MacOSLineDesktopAdapter
...
class Adapter(MacOSLineDesktopAdapter):
    ...
    def execute_ocr_window_title(self, *args, **kwargs):
        return {
            ...
            "payload_summary": {
                "items": [
                    {"role": "ocr_window_header", "text": "メンパー』色"},
                    {"role": "ocr_window_header", "text": "7:58 PM"},
                ],
            },
        }
...
`;
  ...
  assert.equal(result.validation.reason, 'matched');
  assert.deepEqual(result.validation.chat_title_matches, ['ocr_window_header']);
  assert.equal(result.validation.ocr_title_items_sample[0], 'メンパー』色');
});
