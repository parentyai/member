...
test('phase875: execute_send_text can use composer OCR fallback when LINE hides the composer AX field', () => {
  const code = `
import json
...
class Adapter(MacOSLineDesktopAdapter):
    ...
    def execute_ocr_window_title(self, *args, **kwargs):
        return {
            ...
            "payload_summary": {
                "items": [
                    {"role": "ocr_window_header", "text": "Codex LINE Desktop execute smoke test"},
                ],
            },
        }
...
`;
  ...
  assert.equal(result.fallback.press_return.status, 'executed');
});
