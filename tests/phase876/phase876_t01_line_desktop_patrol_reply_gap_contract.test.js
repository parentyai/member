...
test('phase876: execute harness degrades missing visible rows to post_send_reply_missing when post-send target validation still confirms the target', (t) => {
  const tempRoot = makeTempRoot('phase876-reply-gap-');
...
class FakeAdapter:
    ...
    def execute_validate_target(self, **kwargs):
        ...
        if ax_output_path:
            ...
            path_obj.write_text(json.dumps({"window_name": "LINE - メンバー"}), encoding="utf-8")
        ...
        return {
            "status": "executed",
            "ax_dump": {"status": "executed", "output_path": str(ax_output_path), "payload_summary": {"window_name": "LINE - メンバー"}},
