...
test('phase891: synthesize_code_apply_task writes prompt and review checklist', (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'phase891-code-apply-task-'));
  ...
  fs.writeFileSync(path.join(queueRoot, 'queue.jsonl'), `${JSON.stringify({
    proposal_id: 'proposal_code_apply_task_demo',
    source_trace_ids: ['ldp_execute_09'],
  ...
  fs.writeFileSync(path.join(packetRoot, 'proposal_code_apply_task_demo.codex.json'), JSON.stringify({
    proposal_id: 'proposal_code_apply_task_demo',
    trace_ref: { run_id: 'ldp_execute_09', trace_path: '/tmp/trace.json', scenario_id: 'member_only_execute_smoke', target_id: 'member-self-test', failure_reason: 'execute_evaluated' },
    evaluation_ref: { planning_status: 'ready', analysis_status: 'root_cause_identified', observation_status: 'reply_observed' },
    operator_summary: { headline: 'needs apply task', status: 'needs_patch' },
    proposal: {
      title: 'Apply task bundle',
  ...
  const code = `
import json
from member_line_patrol.synthesize_code_apply_task import synthesize_code_apply_task
...
`;
  ...
  const draftJson = JSON.parse(fs.readFileSync(result.code_apply_task_path, 'utf8'));
  const draftMd = fs.readFileSync(result.code_apply_task_markdown_path, 'utf8');
  const promptMd = fs.readFileSync(result.worker_prompt_path, 'utf8');

  assert.equal(result.status, 'ready_for_human_code_apply_task');
  assert.ok(fs.existsSync(result.code_apply_task_path));
  assert.ok(fs.existsSync(result.code_apply_task_markdown_path));
  assert.ok(fs.existsSync(result.worker_prompt_path));
  assert.equal(draftJson.task_packets.length, 1);
  assert.match(draftMd, /Code apply task for proposal_code_apply_task_demo/);
  assert.match(draftMd, /Review checklist/);
  assert.match(promptMd, /final human-reviewed apply_patch execution/);
});
