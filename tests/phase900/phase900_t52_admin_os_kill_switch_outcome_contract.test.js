...
const { handleSet } = require('../../src/routes/admin/osKillSwitch');
...
function req(traceId) {
  return {
    method: 'POST',
    url: '/api/admin/os/kill-switch/set',
    headers: {
...
test('phase900: os kill switch set missing confirm token emits normalized error outcome metadata', async () => {
  const res = createResCapture();
  await handleSet(req('trace_phase900_t52_confirm_required'), res, JSON.stringify({ isOn: true }));
  ...
  assert.equal(body.outcome && body.outcome.guard && body.outcome.guard.routeKey, 'admin_os_kill_switch');
...
test('phase900: os kill switch set invalid json emits normalized error outcome metadata', async () => {
  const res = createResCapture();
  await handleSet(req('trace_phase900_t52_invalid_json'), res, '{');
