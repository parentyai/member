...
const { handleSendPhase1 } = require('../../src/routes/admin/phase1Notifications');
...
function req(traceId) {
  return {
    method: 'POST',
    url: '/admin/phase1/notifications/notif_legacy/send',
    headers: {
...
test('phase900: legacy phase1 notifications send freeze emits blocked outcome headers', async () => {
  await withLegacyFreezeEnabled(async () => {
    const res = createResCapture();
    await handleSendPhase1(req('trace_phase900_t57_send_freeze'), res, JSON.stringify({
