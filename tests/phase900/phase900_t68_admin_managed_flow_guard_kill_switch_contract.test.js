...
test('phase900: managed flow guard blocks with 409 when kill switch confirmation is required but missing', async () => {
  const actionKey = 'managed_flow.kill_switch.execute';
  const flow = {
    flowId: 'flow_kill_switch',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/kill-switch',
        headers: {
          'x-trace-id': 'trace_phase900_guard_kill_switch',
      ...
      actionKey,
      payload: {
        planHash: 'plan_phase900_kill_switch',
        confirmToken: 'confirm_phase900_kill_switch',
    ...
    assert.ok(audits.some((entry) => entry && entry.action === 'managed_flow.guard.violation'
      && entry.payloadSummary && entry.payloadSummary.reason === 'kill_switch_check_required'));
  } finally {
    restore();
  }
});
