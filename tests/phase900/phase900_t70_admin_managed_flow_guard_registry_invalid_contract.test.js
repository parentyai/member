...
function loadGuardWithStubs(registryLoader, resolver) {
  ...
  require.cache[registryPath] = {
    ...
    loaded: true,
    exports: {
      getManagedFlowRegistry: registryLoader
    }
  };
  require.cache[bindingsPath] = {
    ...
    loaded: true,
    exports: {
      resolveActionByMethodAndPath: resolver || (() => null)
    }
  };
...
test('phase900: managed flow guard emits 503 error outcome when registry loading fails', async () => {
  const { guardModule, restore } = loadGuardWithStubs(() => {
    throw new Error('REGISTRY_CORRUPTED_PHASE900');
  });
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/test',
        headers: {
          'x-trace-id': 'trace_phase900_guard_registry_invalid',
          'x-actor': 'phase900_actor'
        }
      },
      res,
      actionKey: 'custom.registry.check',
      payload: {}
    });
    ...
    assert.equal(body.error, 'managed_flow_registry_invalid');
    assert.equal(body.detail, 'REGISTRY_CORRUPTED_PHASE900');
    assert.equal(body.outcome && body.outcome.state, 'error');
    assert.equal(body.outcome && body.outcome.reason, 'managed_flow_registry_invalid');
    assert.equal(res.result.headers['x-member-outcome-state'], 'error');
    assert.equal(res.result.headers['x-member-outcome-reason'], 'managed_flow_registry_invalid');
  } finally {
    restore();
  }
});
