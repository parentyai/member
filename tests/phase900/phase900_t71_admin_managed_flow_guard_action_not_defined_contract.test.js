...
test('phase900: managed flow guard emits 500 error outcome when actionKey is absent from registry', async () => {
  const registry = {
    actionByKey: {},
    flowById: {}
  };
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/action-not-defined',
        headers: {
          'x-trace-id': 'trace_phase900_guard_action_not_defined',
          'x-actor': 'phase900_actor'
        }
      },
      res,
      actionKey: 'custom.action.missing',
      payload: {}
    });
    ...
    assert.equal(body.actionKey, 'custom.action.missing');
    assert.equal(body.outcome && body.outcome.state, 'error');
    assert.equal(body.outcome && body.outcome.reason, 'managed_flow_action_not_defined');
