...
test('phase900: managed flow guard falls back to warn_only confirmMode when flow.confirmMode is absent and guardRules.confirmMode is optional', async () => {
  const actionKey = 'managed_flow.confirm.fallback.warn_only';
  const flow = {
    flowId: 'flow_confirm_warn_fallback',
    guardRules: {
      actorMode: 'required',
      traceMode: 'required',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/confirm-fallback-warn',
        headers: {
          'x-trace-id': 'trace_phase900_confirm_warn',
