...
test('phase900: managed flow guard uses x-request-id as fallback trace id when allow_fallback flow has no trace header', async () => {
  const actionKey = 'managed_flow.trace_fallback.request_id';
  const flow = {
    flowId: 'flow_trace_fallback_request_id',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/trace-fallback-request-id',
        headers: {
          'x-request-id': 'req_phase900_t82',
      ...
      actionKey,
      payload: {
        planHash: 'plan_phase900_t82',
        confirmToken: 'confirm_phase900_t82'
      }
    }, {
    ...
    assert.equal(result.traceId, 'req_phase900_t82');
    ...
    assert.equal(traceWarning.traceId, 'req_phase900_t82');
  } finally {
    restore();
  }
});
