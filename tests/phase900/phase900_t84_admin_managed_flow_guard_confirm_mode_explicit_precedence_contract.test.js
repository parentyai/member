...
test('phase900: managed flow guard respects explicit flow.confirmMode over guardRules.confirmMode fallback', async () => {
  const actionKey = 'managed_flow.confirm_mode.explicit_precedence';
  const flow = {
    flowId: 'flow_confirm_mode_explicit_precedence',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/confirm-mode-explicit-precedence',
        headers: {
          'x-trace-id': 'trace_phase900_t84',
