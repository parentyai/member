...
test('phase900: managed flow guard treats whitespace x-trace-id as missing and uses x-request-id fallback in allow_fallback mode', async () => {
  const actionKey = 'managed_flow.trace_whitespace.request_fallback';
  const flow = {
    flowId: 'flow_trace_whitespace_request_fallback',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/trace-whitespace-request-fallback',
        headers: {
          'x-trace-id': '   ',
          'x-request-id': 'req_phase900_t92',
      ...
      actionKey,
      payload: {
        planHash: 'plan_phase900_t92',
        confirmToken: 'confirm_phase900_t92'
      }
    }, {
    ...
    assert.equal(result.traceId, 'req_phase900_t92');
    ...
    assert.deepEqual(warningReasons, ['trace_fallback']);
  } finally {
    restore();
  }
});
