...
test('phase900: managed flow guard treats whitespace x-actor as missing and falls back to unknown in allow_fallback mode', async () => {
  const actionKey = 'managed_flow.actor.whitespace_fallback';
  const flow = {
    flowId: 'flow_actor_whitespace_fallback',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/actor-whitespace-fallback',
        headers: {
          'x-trace-id': 'trace_phase900_t94',
          'x-actor': '   '
        }
      },
      res,
      actionKey,
      payload: {
        planHash: 'plan_phase900_t94',
        confirmToken: 'confirm_phase900_t94'
      }
    }, {
    ...
    assert.equal(result.traceId, 'trace_phase900_t94');
