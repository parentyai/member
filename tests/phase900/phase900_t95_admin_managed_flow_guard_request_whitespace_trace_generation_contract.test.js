...
test('phase900: managed flow guard ignores whitespace x-request-id and generates managed trace fallback in allow_fallback mode', async () => {
  const actionKey = 'managed_flow.trace.request_whitespace_fallback';
  const flow = {
    flowId: 'flow_trace_request_whitespace_fallback',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/trace-request-whitespace-fallback',
        headers: {
          'x-trace-id': '   ',
          'x-request-id': '   ',
      ...
      actionKey,
      payload: {
        planHash: 'plan_phase900_t95',
        confirmToken: 'confirm_phase900_t95'
      }
    }, {
    ...
    assert.match(result.traceId, /^managed_flow_trace_/);

    const traceWarning = audits.find((entry) => (
      entry
      && entry.action === 'managed_flow.guard.warning'
      && entry.payloadSummary
      && entry.payloadSummary.reason === 'trace_fallback'
    ));
    assert.ok(traceWarning);
    assert.equal(traceWarning.traceId, result.traceId);
  } finally {
    restore();
  }
});
