...
test('phase900: managed flow guard treats whitespace planHash/confirmToken as missing and blocks in required confirm mode before kill switch checks', async () => {
  const actionKey = 'managed_flow.confirm.whitespace_required';
  const flow = {
    flowId: 'flow_confirm_whitespace_required',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/confirm-whitespace-required',
        headers: {
          'x-trace-id': 'trace_phase900_t97',
