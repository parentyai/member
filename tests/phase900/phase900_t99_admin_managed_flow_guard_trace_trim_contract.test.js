...
test('phase900: managed flow guard trims x-trace-id and keeps trimmed trace value in success path', async () => {
  const actionKey = 'managed_flow.trace.trim_required';
  const flow = {
    flowId: 'flow_trace_trim_required',
  ...
  try {
    ...
    const result = await guardModule.enforceManagedFlowGuard({
      req: {
        method: 'POST',
        url: '/api/admin/custom/trace-trim-required',
        headers: {
          'x-trace-id': '   trace_phase900_t99   ',
      ...
      actionKey,
      payload: {
        planHash: 'plan_phase900_t99',
        confirmToken: 'confirm_phase900_t99'
      }
    }, {
    ...
    assert.equal(result.traceId, 'trace_phase900_t99');
