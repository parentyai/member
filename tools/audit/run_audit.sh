...
run_step "structure risk budget check" bash -lc "cd \"$ROOT_DIR\" && npm run structure-risk:check > \"$OUT_DIR/structure-risk-check.txt\"" \
  || fail "structure risk check failed"
