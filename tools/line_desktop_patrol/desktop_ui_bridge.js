...
function evaluateConversationLoop(input) {
  ...
  const expectedReplyMatched = sendMode === 'dry_run'
    ...
    : expectedReplySubstrings.length === 0
      ? replyObserved
      : expectedReplySubstrings.some((item) => finalTranscript.includes(item) || replyJoined.includes(item));
  const forbiddenReplyHit = forbiddenReplySubstrings.some((item) => finalTranscript.includes(item) || replyJoined.includes(item));
