...
function buildReadme({ bundleRoot, targetAlias, targetChatTitle }) {
  ...
  return [
    ...
    '2. Run `npm run line-desktop-patrol:doctor` first and confirm the bundle is ready.',
    '3. Run `npm run line-desktop-patrol:open-target` and continue only if it returns `open_target_ready`.',
    '4. Run `npm run line-desktop-patrol:execute-once` after the preflight passes.',
    '5. Run `npm run line-desktop-patrol:loop-execute` only after `execute-once` works.',
    '6. Finish with `npm run line-desktop-patrol:acceptance-gate` against the soak report.',
    '',
    '## Debug commands',
    '- `line-desktop-patrol:send` is debug-only and stays outside the formal safe sequence.',
    '',
    '## Local execute toggle',
    'Keep `enabled=false` and `allowed_send_modes=["dry_run"]` until `doctor` and `open-target` succeed.',
    'When ready for a supervised execute test, change only this local file:',
    '   - `enabled=true`',
    ...
    `   - \`npm run line-desktop-patrol:loop-execute -- --policy ${path.join(bundleRoot, 'soak', 'policy.soak.json')} --scenario ${path.join(bundleRoot, 'scenarios', 'execute_smoke.json')}\``,
    `   - \`npm run line-desktop-patrol:acceptance-gate -- --manual-report ${path.join(bundleRoot, 'acceptance.manual.json')}\``,
    'For soak, copy the same execute-ready settings into `soak/policy.soak.json` and keep the same member-only target.',
    'If `open-target` returns `open_target_mismatch_stop`, do not continue to `execute-once` or `loop-execute`.',
    'Treat `generic LINE shell only` as `frontmost=true`, `window_name="LINE"`, `visible_item_count=0`, and header OCR empty-or-timeout. The bundle should still fail closed until the member-only chat is visible.',
    `Pin the target to \`${titleLabel}\` and do not widen the allowlist beyond that member-only self-test chat.`,
    '',
    '## Stop',
