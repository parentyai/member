...
function runReplay(rows) {
  ...
  list.forEach((row) => {
    ...
    if (!expected || !observed) {
      failures.push({ id: row && row.id ? row.id : 'unknown', reason: 'missing_expected_or_observed' });
      return;
    }
    if (expected !== observed) {
      if (expected === 'warning' || observed === 'warning') {
        warnings.push({ id: row.id, expected, observed, scenario: row.scenario || 'unknown' });
      } else {
        failures.push({ id: row.id, expected, observed, scenario: row.scenario || 'unknown' });
      }
    }
  });
  return {
    ...
    warningFailures: warnings.length,
    failures,
    warnings
  };
}
...
function main(argv) {
  ...
  const fixturePath = args.fixture
    ...
    : path.join(__dirname, 'fixtures', 'trace_replay_cases.v1.json');
  const outputPath = args.output
    ? path.resolve(process.cwd(), args.output)
    : path.join(process.cwd(), 'tmp', 'llm_replay_result.json');
  const rows = readJson(fixturePath);
  const result = runReplay(rows);
